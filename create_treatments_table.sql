...
-- Policy: Users can view treatments for patients they have access to
CREATE POLICY "Users can view treatments for their patients" ON public.treatments
    ...
    USING (
        patient_id IN (
            ...
            OR id IN (
                SELECT patient_id FROM public.patient_professionals 
                WHERE professional_id = auth.uid()
            )
        )
    );

-- Policy: Users can insert treatments for patients they have access to
...
-- Policy: Users can update treatments for patients they have access to
...
-- Policy: Users can delete treatments for patients they have access to
