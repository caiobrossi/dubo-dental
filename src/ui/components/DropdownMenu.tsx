      ...
      <SubframeCore.DropdownMenu.Item asChild={true} {...otherProps}>
        <div
          className={SubframeUtils.twClassNames(
            "group/adcae8d6 flex h-8 w-full cursor-pointer items-center gap-2 rounded-md px-3 hover:bg-new-gray-4 active:bg-new-gray-6 data-[highlighted]:bg-brand-50",
            className
          )}
          ref={ref}
        >
          ...
          {children ? (
            <span className="line-clamp-1 grow shrink-0 basis-0 text-body-medium font-body-medium text-default-font group-hover/adcae8d6:text-default-font">
              {children}
            </span>
          ) : null}
        </div>
