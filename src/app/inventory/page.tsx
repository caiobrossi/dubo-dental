...
import WithdrawProductModal from "@/components/custom/WithdrawProductModal";
...
function InventoryPage() {
  ...
  // Update column filters when category changes
  useEffect(() => {
  ...
  const handleWithdrawClick = (product: InventoryItem) => {
  ...
  // Handle scroll in table to hide/show stats
  const handleTableScroll = (event: React.UIEvent<HTMLDivElement>) => {
    const scrollTop = event.currentTarget.scrollTop;
    
    // Hide stats when scrolled down more than 50px
    if (scrollTop > 50 && !hideStats) {
      setHideStats(true);
    }
    // Show stats when scrolled back to near top (less than 20px)
    else if (scrollTop <= 20 && hideStats) {
      setHideStats(false);
    }
  };
  ...
  const getStockStatus = (current: number, min: number, max: number): "success" | "error" | "neutral" => {
    if (current < min) return "error";        // Red: Below minimum
    if (current > max) return "success";      // Green: Above maximum  
    return "neutral";                         // Neutral: Between min and max
  };
    ...
    () => [
      ...
      columnHelper.accessor("current_stock", {
        ...
        cell: (info) => {
          ...
          const current = Number(current_stock);
          const min = Number(minimum_stock);
          const max = Number(maximum_stock);
        ...
        cell: (info) => (
          <div className="flex items-center justify-end gap-2">
            <Button
              ...
              size="large"
              onClick={() => handleWithdrawClick(info.row.original)}
            >
              Withdraw
            </Button>
  ...
  return (
    <DefaultPageLayout>
      <div className="flex h-full w-full flex-col items-start gap-4 bg-default-background shadow-md pb-3">
        ...
        <div 
          className={`w-full bg-default-background px-8 transition-all duration-300 ease-in-out overflow-hidden ${
            hideStats 
          ...
          <div className="flex w-full items-start justify-center gap-2">
            ...
            <div className="flex grow shrink-0 basis-0 items-center gap-8 border-r border-solid border-neutral-border px-1 py-1">
              ...
              <div className="flex flex-col items-end gap-4">
                ...
                <span className="font-['Urbanist'] text-[60px] font-[300] leading-[36px] text-default-font">
                  {data.filter(item => item.current_stock <= item.minimum_stock).length}
                </span>
              </div>
            </div>
            <div className="flex grow shrink-0 basis-0 items-center gap-8 border-r border-solid border-neutral-border px-1 py-1">
              ...
              <div className="flex flex-col items-end gap-4">
                ...
                <span className="font-['Urbanist'] text-[60px] font-[300] leading-[36px] text-default-font">
                  {data.filter(item => item.current_stock > item.minimum_stock && item.current_stock < item.maximum_stock).length}
                </span>
              </div>
            </div>
            <div className="flex grow shrink-0 basis-0 items-center gap-8 px-1 py-1">
              ...
              <div className="flex flex-col items-end gap-4">
                ...
                <span className="font-['Urbanist'] text-[60px] font-[300] leading-[36px] text-default-font">
                  {data.filter(item => item.current_stock >= item.maximum_stock).length}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className={`flex w-full flex-wrap items-center justify-between px-8 transition-all duration-300 ${
          hideStats ? 'pb-2 pt-2' : 'pb-4'
        }`}>
        ...
        <div className={`flex w-full grow shrink-0 basis-0 flex-col items-stretch gap-2 rounded-lg bg-default-background px-8 overflow-auto transition-all duration-300 ${
          hideStats ? 'pb-3 pt-1' : 'pb-6'
        }`}>
